...
def _upsample_like(src, tar):
    ...
    src = F.upsample(src, size=tar.shape[2:], mode="bilinear", align_corners=False) # add align_corners to make sure is compatible with older versions
...
class U2NETS(nn.Module):
    """ This is a smaller version of U2NET.

        RSU7 was eliminated so that even with input size of 128x128, the last layers are not empty.
        Also, middle channels were reduced in size.
    ...
    def __init__(self, in_ch=3, out_ch=1):
        ...
        self.stage1 = RSU6(in_ch, 32, 64)
        self.pool12 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage2 = RSU5(64, 64, 128)
        self.pool23 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage3 = RSU4(128, 128, 256)
        self.pool34 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage4 = RSU4F(256, 256, 256)
        self.pool45 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage5 = RSU4F(256, 256, 256)

        # decoder
        self.stage4d = RSU4F(512, 256, 256)
        self.stage3d = RSU4(512, 128, 128)
        self.stage2d = RSU5(256, 64, 64)
        self.stage1d = RSU6(128, 32, 64)
    ...
    def forward(self, x):
        ...
        """
        del hx1, hx2, hx3, hx4, hx5
        del hx4d, hx3d, hx2d, hx1d
        del hx5dup, hx4dup, hx3dup, hx2dup
        """
